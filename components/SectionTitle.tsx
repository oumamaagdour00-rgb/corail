...
const SectionTitle: React.FC<SectionTitleProps> = ({ title, subtitle, alignment = 'left', light = false }) => {
  ...
  return (
    <div className={`mb-12 ${alignment === 'center' ? 'text-center' : 'text-left'}`}>
      <h2 className={`text-2xl sm:text-3xl md:text-4xl xl:text-5xl font-display font-black my-10 ${light ? 'text-white' : isArabic ? 'text-corail-900' : 'bg-clip-text text-transparent bg-gradient-to-r from-corail-900 via-corail-800 to-corail-600'}`}>
        {title}
      </h2>
      {subtitle && (
